    ...
    <el-card>
      ...
      <el-row :gutter="5">
        ...
        <el-col :span="2" :offset="8">
          <el-button type="danger" v-if="multipleButtonFlag" @click="multiDelete">批量删除</el-button>
        </el-col>
      </el-row>
      ...
      >
        ...
        <el-table-column label="性别" prop="sex" :formatter="formatSex"/>
        <el-table-column label="学院" prop="collegeName"/>
        <el-table-column label="年级" prop="level" :formatter="formatLevel"/>
  ...
  export default {
    ...
    methods: {
      ...
      formatLevel(row, column) {
        ...
        return year - row.level === 1 ? '大一'
            : year - row.level === 2 ? '大二'
                : year - row.level === 3 ? '大三' : '大四';
      ...
      async showEditDialog(id) {
        ...
        if (res.code !== 200) {
          return this.$message.error("查询用户信息失败！请重试");
        }
      ...
      handleSelectionChange(val) {
        ...
        this.multipleButtonFlag = true;

        if (this.multipleSelection.length === 0) {
          // 如不进行判断则勾选完毕后批量删除按钮还是会在
          this.multipleButtonFlag = false;
        }
      },
